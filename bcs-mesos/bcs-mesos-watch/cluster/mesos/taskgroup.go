...
func (task *TaskGroupWatch) AddEvent(obj interface{}) {
	...
	if err := task.report.ReportData(data); err != nil {
		util.ReportSyncTotal(cluster.DataTypeTaskGroup, types.ActionAdd, cluster.SyncFailure)
	} else {
		util.ReportSyncTotal(cluster.DataTypeTaskGroup, types.ActionAdd, cluster.SyncSuccess)
	}
}
...
func (task *TaskGroupWatch) DeleteEvent(obj interface{}) {
	...
	if err := task.report.ReportData(data); err != nil {
		util.ReportSyncTotal(cluster.DataTypeTaskGroup, types.ActionDelete, cluster.SyncFailure)
	} else {
		util.ReportSyncTotal(cluster.DataTypeTaskGroup, types.ActionDelete, cluster.SyncSuccess)
	}
}
...
func (task *TaskGroupWatch) UpdateEvent(old, cur interface{}, force bool) {
	...
	if err := task.report.ReportData(data); err != nil {
		util.ReportSyncTotal(cluster.DataTypeTaskGroup, types.ActionUpdate, cluster.SyncFailure)
	} else {
		util.ReportSyncTotal(cluster.DataTypeTaskGroup, types.ActionUpdate, cluster.SyncSuccess)
	}
}
