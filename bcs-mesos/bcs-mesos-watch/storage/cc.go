...
func (cc *CCStorage) Sync(data *types.BcsSyncData) error {
	...
	cc.queue <- data
	util.ReportHandlerQueueLengthInc(handlerCCStore)
	return nil
}
...
func (cc *CCStorage) SyncTimeout(data *types.BcsSyncData, timeout time.Duration) error {
	...
	case <-time.After(timeout):
		...
		util.ReportHandlerDiscardEvents(handlerCCStore)
	}
...
func (cc *CCStorage) Worker() {
	...
	for {
		...
		case <-cc.exitCxt.Done():
			...
			return
		case data := <-cc.queue:
			util.ReportHandlerQueueLengthDec(handlerCCStore)
...
func (cc *CCStorage) reportCCStoreQueueLength() {
	...
	for {
		...
		util.ReportHandlerQueueLength(handlerCCStore, float64(len(cc.queue)))
	}
}
